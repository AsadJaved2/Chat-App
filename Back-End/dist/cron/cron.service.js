"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MessageArchiveService = void 0;
const common_1 = require("@nestjs/common");
const schedule_1 = require("@nestjs/schedule");
const chat_gateway_1 = require("../chat/chat-gateway");
const fs = require("fs");
const path = require("path");
let MessageArchiveService = class MessageArchiveService {
    constructor(chatGateway) {
        this.chatGateway = chatGateway;
    }
    handleCron() {
        this.archiveMessages();
    }
    archiveMessages() {
        const messages = this.chatGateway['messages'];
        const filePath = path.join(__dirname, 'messages.txt');
        const messageStrings = messages.map(msg => {
            return `[${msg.timestamp.toISOString()}] ${msg.userId}: ${msg.message}`;
        });
        fs.appendFileSync(filePath, messageStrings.join('\n') + '\n', 'utf8');
        this.chatGateway['messages'] = [];
    }
};
exports.MessageArchiveService = MessageArchiveService;
__decorate([
    (0, schedule_1.Cron)('0 */2 * * *'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], MessageArchiveService.prototype, "handleCron", null);
exports.MessageArchiveService = MessageArchiveService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [chat_gateway_1.ChatGateway])
], MessageArchiveService);
//# sourceMappingURL=cron.service.js.map